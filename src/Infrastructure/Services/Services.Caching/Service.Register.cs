        ...
        )
        {
            ...
            if (redisConfiguration is null)
                throw new ArgumentNullException(
                    ...
                    "Redis configuration is not set."
                );

            if (string.IsNullOrWhiteSpace(redisConfiguration.Connection))
                throw new ArgumentException(
                    "Redis connection string is not set.",
                    nameof(redisConfiguration.Connection)
                );

            if (string.IsNullOrWhiteSpace(redisConfiguration.InstanceName))
                throw new ArgumentException(
                    "Redis instance name is not set.",
                    nameof(redisConfiguration.InstanceName)
                );

            if (redisConfiguration.Database < 0)
                throw new ArgumentOutOfRangeException(
                    nameof(redisConfiguration.Database),
                    "Redis database number must be non-negative."
                );
